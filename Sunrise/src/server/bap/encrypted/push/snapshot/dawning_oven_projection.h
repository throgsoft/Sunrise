...
[[nodiscard]] inline bool project_socket_result(const state::PendingSocketPlug& mutation,
    ...
    if (!state::build_data::find_item_definition_hash(gain->definitionHash, cookie)) {
        return false;
    }
    std::size_t matches = 0;
    for (const auto& item : object.profileItems) {
        if (item.mutationSerial != gain->mutationSerial) {
            continue;
        }
