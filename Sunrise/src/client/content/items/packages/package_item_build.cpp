    ...
    if (investment_globals_tags(candidates, count)) {
        for (std::size_t i = 0; i < count && !configured; ++i) {
            ...
            if (!reader::read_tag(source, storage.scratch, candidates[i], storage.container)
                ...
                || root == 0 || !reader::read_tag(source, storage.scratch, root, storage.root, rootClass)
                || rootClass != tables::kInvestmentRootClass) continue;
            ...
            if (!readBanks || !chalice::configure_banks(banks[0], banks[1], banks[2], banks[3])
                ...
                || !reader::read_tag(source, storage.scratch, tag, storage.itemIndexTable)) continue;
            tables::Array items{};
            ...
            for (std::uint16_t index = 7937; index <= 8019; ++index) {
                ...
                tables::IndexRow row{};
                if (!tables::index_row(storage.itemIndexTable, items, index, row) || row.targetTag == 0
...
bool configure_mote_visibility(const reader::Source& source, Storage& storage) noexcept {
    ...
    if (!tables::slot_tag(storage.root, tables::kItemTableSlot, itemTag) || itemTag == 0
        || !tables::slot_tag(storage.root, tables::kUnlockFlagSlotTableSlot, flagTag) || flagTag == 0
    ...
    if (investment_globals_tags(candidates, count)) {
        for (std::size_t i = 0; i < count && (!configured || !visibility); ++i) {
            ...
            if (!reader::read_tag(source, storage.scratch, candidates[i], storage.container)
                ...
                continue;
            if (!configured
                && tables::slot_tag(storage.root, tables::kSocketTypeTableSlot, table)
                && table != 0 && reader::read_tag(source, storage.scratch, table, storage.child))
                configured = state::runtime::detail::synthesizer::configure_socket_costs(storage.child);
...
bool ready() noexcept {
    return g_synthesizerMetadataAttempted.load(std::memory_order_acquire)
           && g_chaliceMetadataAttempted.load(std::memory_order_acquire)
           && root_domains_ready() && state::build_data::scenario_layouts_ready()
...
bool build() noexcept {
    ...
    if (investment_globals_tags(candidates, candidateCount)) {
        ...
        for (std::size_t candidate = 0; candidate < candidateCount && !located; ++candidate) {
            ...
            // Item details, records, nodes, season pass rewards and catalysts resolve slots
            // through the two unlock mapping tables, so they are read once here.
