...
[[nodiscard]] inline bool weapon_slot_matches(const std::optional<gameplay::WeaponSlot>& required,
                                              const gameplay::Event& event) noexcept {
    if (!required) return true;
    return gameplay::valid_weapon_slot(*required) && event.weaponKill.value_or(false)
           && event.weaponSlot && *event.weaponSlot == *required && !event.meleeKill.value_or(false)
           && !event.grenadeKill.value_or(false) && !event.superKill.value_or(false)
           && !event.abilityKill.value_or(false);
}

[[nodiscard]] inline bool
source_alternatives_match(const std::optional<SourceAlternatives>& alternatives,
                          const gameplay::Event& event) noexcept {
    if (!alternatives) return true;
    if (alternatives->weaponSlot && weapon_slot_matches(alternatives->weaponSlot, event))
        return true;
    if ((alternatives->grenade && event.grenadeKill.value_or(false))
        ...
        || (alternatives->ability && event.abilityKill.value_or(false)))
        return true;
    if (!event.weaponKill.value_or(false) || !event.weaponClass || *event.weaponClass == 0)
        return false;
    return std::any_of(alternatives->weaponClasses.begin(),
                       alternatives->weaponClasses.end(),
                       [&](std::uint32_t hash) { return hash != 0 && hash == *event.weaponClass; });
}

/** Stages eligible objective changes. Persistence and replay admission belong to the caller. */
[[nodiscard]] inline GameplayKillResult
apply_gameplay_kill_transaction(AccountState& accountState,
                                const gameplay::Event& event,
                                gameplay::Context currentContext,
                                std::uint16_t authoritativeActivity,
                                std::span<const KillRuleDefinition> catalog,
                                std::int64_t now,
    ...
    if (mappingValid == nullptr || validate == nullptr
        || accountState.characterCount > accountState.characters.size()
        || !validate(accountState)) {
    ...
    if (accountState.characters[characterIndex].currentActivityIndex != authoritativeActivity) {
        result.status = GameplayKillStatus::invalidContext;
        return result;
    }
    ...
    const auto candidate =
        std::unique_ptr<AccountState>(new (std::nothrow) AccountState(accountState));
    if (!candidate) {
    ...
    for (std::size_t i = 0; i < inventory.count; ++i) {
        ...
        for (auto rule : catalog) {
            if (rule.itemHash != held.definitionHash || rule.itemHash == 0 || !mappingValid(rule)
                ...
                || (rule.victimClassHash != 0
                    && (!event.victimClassHash || *event.victimClassHash != rule.victimClassHash))
                || !weapon_slot_matches(rule.weaponSlot, event)
                || (rule.victimRanks != 0
                    && ((rule.victimRanks & ~std::uint8_t{31}) != 0 || !event.victimRankHash
                        || (rule.victimRanks & gameplay::rank_bit(*event.victimRankHash)) == 0))
                || (rule.weaponClass != 0
                    && (!event.weaponKill.value_or(false) || !event.weaponClass
                        || *event.weaponClass != rule.weaponClass))
                ...
                || !source_alternatives_match(rule.anySource, event) || rule.contribution <= 0
                || (rule.provisionalRankPoints
                    && (rule.contribution != 1 || rule.precisionContribution
                        || !event.victimRankHash
                        || !gameplay::provisional_rank_points(*event.victimRankHash)))
                || (rule.precisionContribution
                    && (*rule.precisionContribution < rule.contribution
                        || !event.precision.has_value())))
                continue;
            ...
            after.values[i].objectiveValues[rule.lane] =
                before + std::min(contribution, rule.completion - before);
