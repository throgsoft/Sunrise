...
bool pickup_space(const CharacterState& character, const buckets::Descriptor& bucket,
    ...
    for (std::size_t i = 0; i < character.inventory.count; ++i) {
        ...
        if (item.placement == account::inventory::ItemPlacement::inventory && !count(item.definitionHash))
            return false;
    }
 ...
 */
bool insert_pickups(CharacterState& character, std::uint32_t hash, std::int32_t quantity,
                    std::int32_t& lastSerial) noexcept {
    buckets::Descriptor bucket{};
    if (quantity <= 0 || !pickup_bucket(hash, bucket)
...
bool same_pickups(const CharacterState& expected, const CharacterState& after) noexcept {
    const auto next = [](const CharacterState& character, std::size_t& i)
        -> const account::inventory::CharacterStack* {
...
MaterialReward stage_reward(CharacterState&,
    ...
    buckets::Descriptor bucket{};
    if (credited > 0 && !pickup_bucket(result.definitionHash, bucket)) return MaterialReward::refused;
...
bool validate_rewards(const PendingRecordRewardGrant& mutation) noexcept {
    ...
    auto expectedCharacter = std::unique_ptr<CharacterState>{
        mutation.beforeDawning ? new (std::nothrow) CharacterState(mutation.beforeCharacter) : nullptr};
...
bool stage_queued_pickups(std::uint64_t characterSoid, std::size_t& acquired) noexcept {
    ...
    if (!transaction.ready() || !current || !store::read_account(*current)
        || !account::valid(*current)) return false;
    ...
    for (std::size_t i = 0; i < character->stacks.count; ++i) {
        ...
        if (index < identity::kIngredientCount && row.definitionHash == identity::kIngredients[index].pickupHash)
    ...
    while (acquired < capacity) {
        ...
        std::int32_t quantity{};
        {
            ...
            if (status != SQLITE_ROW || !row.columns(id, hash, quantity) || quantity <= 0) return false;
        }
        buckets::Descriptor bucket{};
        ...
        if (count == quantity) {
            store::Statement erase("DELETE FROM dawning_pickup_queue WHERE id=? AND character_soid=?");
            if (!erase.write(id, characterSoid) || sqlite3_changes(store::g_database) != 1) return false;
        } else {
            store::Statement reduce("UPDATE dawning_pickup_queue SET quantity=? WHERE id=? AND character_soid=?");
            if (!reduce.write(quantity - count, id, characterSoid) || sqlite3_changes(store::g_database) != 1)
                return false;
        }
        acquired += count;
    }
...
bool drain_pickups(std::uint64_t characterSoid, std::size_t& removed) noexcept {
    ...
    std::fill(rows.values.begin() + retained, rows.values.end(), account::inventory::CharacterStack{});
    rows.count = retained;
    return account::valid(*current) && store::write_account(*current) && transaction.commit();
}
} // namespace sunrise::state::runtime::detail::dawning
