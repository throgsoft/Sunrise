...
bool card(const char* id, const Card& card, bool selected, ImVec2 size, bool showName) {
    ...
    if (texture != ImTextureID_Invalid)
        draw->AddImage(ImTextureRef(texture), start, end);
    else
    ...
    if (card.quantity > 1) {
        ...
        draw->AddRectFilled({pos.x - 2, pos.y}, end, IM_COL32(0, 0, 0, 210));
        draw->AddText(pos, IM_COL32_WHITE, count);
    }
    if (showName) {
...
int grid(const char* id, std::span<const Card> cards, int selected, float maxHeight) {
    ...
    float viewport =
        (std::min)(available.y, (48 / columns - 1) * pitch + style.WindowPadding.y * 2);
    if (maxHeight > 0) viewport = (std::min)(viewport, maxHeight);
    ImGui::BeginChild(id, {0, (std::max)(1.0f, viewport)}, ImGuiChildFlags_Borders);
    if (cards.empty()) ImGui::TextDisabled("No items.");
    ...
    while (clipper.Step()) {
        for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; ++row) {
            for (int col = 0; col < columns; ++col) {
                ...
                if (i >= static_cast<int>(cards.size())) break;
                if (col) ImGui::SameLine();
                const auto& value = cards[i];
                ImGui::PushID(i);
                if (card("##card", value, selected == i, {width, height})) selected = i;
