...
/** The first matching installed transfer rule controls a vendor claim. */
[[nodiscard]] static bool accepted_claim_rule(std::uint16_t vendorIndex,
                                              std::uint8_t sourceBucket) noexcept {
    if (sourceBucket != buckets::kPostmasterBucketId) {
        return false;
    }
    build_data::vendors::IndexEntry index{};
    build_data::vendors::Definition vendor{};
    if (!build_data::vendors::find_index(vendorIndex, index)
        || !build_data::vendors::find(index.definitionHash, vendor) || vendor.index != vendorIndex
...
/** Resolve an instance's authored destination before overflowing or claiming it. */
[[nodiscard]] static bool ordinary_item(const inventory::Item& item,
    ...
    return build_data::find_item_definition_hash(item.definitionHash, definition)
           ...
           && bucket.bucketId != buckets::kPostmasterBucketId
           && bucket.bucketId != buckets::kReceiptBucketId && bucket.slotCount != 0;
}
...
/** Prepare a whole-instance move only when its authored bucket has room. */
[[nodiscard]] static bool stage_claim(const AccountState& before,
    ...
    build_data::items::Definition definition{};
    buckets::Descriptor destination{};
    buckets::Descriptor postmaster{};
...
/** Rebuild the claim under the transaction lock against the saved before-image. */
[[nodiscard]] static bool materialize_claim(const AccountState& current,
    ...
    if (!canonical
        || !stage_claim(current,
                        mutation.vendorIndex,
                        mutation.sourceBucket,
                        mutation.instanceSoid,
                        mutation.definitionIndex,
                        *canonical)
...
bool prepare_postmaster_claim(std::uint16_t vendorIndex,
                              std::uint8_t sourceBucket,
                              std::uint64_t instanceSoid,
                              std::uint16_t definitionIndex,
    ...
    return before && investment::store::read_account(*before)
           && stage_claim(
               *before, vendorIndex, sourceBucket, instanceSoid, definitionIndex, mutation);
}

bool preview_postmaster_claim(const PendingPostmasterClaim& mutation,
                              AccountState& after) noexcept {
...
bool place_instanced_reward(const AccountState& before,
    ...
    if (!scratch || !loadout::resolve(before, characterIndex, scratch->resolved)
        || !loadout::resolve_item(item,
                                  before.characters[characterIndex],
    ...
    const auto& character = before.characters[characterIndex];
    BucketAdmission home;
    if (!character_bucket_admission(character, bucket, home)) {
        return false;
    }
    std::size_t insertion = kCharacterAdmissionCapacity;
    if (home.select(bucket, kCharacterAdmissionCapacity, insertion)) {
        if (insertion == kCharacterAdmissionCapacity) {
            return true;
        }
        if (insertion < kCharacterStackAdmissionBase) {
            evictedInstanceSoid = character.inventory.values[insertion].instanceSoid;
        }
        return erase_character_bucket_row(after, insertion);
    }
    if (home.count != bucket.slotCount
        || (bucket.policyFlags & buckets::kNoTransferOnEviction) != 0) {
        return false;
    }
    build_data::items::Definition definition{};
    buckets::Descriptor destination{};
    ...
    BucketAdmission lostItems;
    if (!character_bucket_admission(character, postmaster, lostItems)
        || !lostItems.select(postmaster, kCharacterAdmissionCapacity, insertion)) {
        return false;
    }
    if (insertion != kCharacterAdmissionCapacity) {
        if (insertion >= character.inventory.count) {
            return false;
        }
        evictedInstanceSoid = character.inventory.values[insertion].instanceSoid;
        if (!erase_character_bucket_row(after, insertion)) {
            return false;
        }
    }
