...
bool premium(const state::build_data::season_pass::Reward& row) noexcept {
    ...
    // Premium rows start with the account-or-profile pass ownership condition.
    return row.conditionCount >= 3
           && row.condition[0].opcode == static_cast<std::uint32_t>(Read::accountFlag)
           && row.condition[1].opcode == static_cast<std::uint32_t>(Read::profileFlag)
           && row.condition[2].opcode
                  == static_cast<std::uint32_t>(
                      middleware::content::packages::tables::UnlockOpcode::logicalOr);
}
...
const char* operation(std::uint32_t opcode) noexcept {
    ...
    using O = middleware::content::packages::tables::UnlockOpcode;
    switch (opcode) {
    ...
    case static_cast<std::uint32_t>(R::profileFlag):
        return "Profile flag";
    case static_cast<std::uint32_t>(R::characterClass):
...
void pass_details(const Catalog& catalog, int index) {
    ...
    if (g_passState.progressFlag) {
        ...
        if (ImGui::Button("Claim")) {
            const bool queued =
                service::grant_season_pass_reward(static_cast<std::uint16_t>(index));
