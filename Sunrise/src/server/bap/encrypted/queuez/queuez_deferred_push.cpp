...
bool take_developer_grant(std::uint64_t accountSoid, DeveloperGrantRequest& output) noexcept {
    ...
    for (auto& request : g_developerGrants)
        if (request.receipt.status == DeveloperGrantStatus::queued
            && (!first || request.receipt.id < first->receipt.id)) first = &request;
...
void finish_developer_grant(const DeveloperGrantRequest& request,
    ...
    core::log::writef(core::log::Channel::server,
                      ...
                      static_cast<unsigned long long>(receipt.id), unsigned(request.definitionIndex),
                      request.quantity, unsigned(receipt.status), receipt.changed,
                      receipt.inventoryCount, receipt.postmasterCount, receipt.materialCount,
...
DeveloperGrantReceipt enqueue_developer_item_grant(std::uint16_t index, std::int32_t quantity,
    ...
    for (auto& candidate : g_developerGrants) {
        if (candidate.receipt.status == DeveloperGrantStatus::queued
            || candidate.receipt.status == DeveloperGrantStatus::publishing) continue;
 ...
 */
[[nodiscard]] bool consume_developer_item_grant(Session& session, Scratch& scratch,
                                                std::span<std::byte> response,
                                                std::size_t& written,
                                                bool& touchesScratch) noexcept {
    if (!session.queuez.family4Active
        || GetTickCount64() < bap::acquisition_queue_deadline()) return false;
    ...
    if (!transaction.ready()) return refuse("investment transaction unavailable; nothing committed");
    ...
    const auto result = state::developer::prepare_item_grant(
        request.definitionIndex, request.quantity, request.definitionHash, request.target, *pending);
    ...
    for (std::size_t i = 0; i < pending->rewardCount; ++i) {
        ...
        if (reward.kind == state::RecordRewardKind::characterInstance || reward.appendedProfileResident)
            residents[residentCount++] = reward.instanceSoid;
    }
    queuez::RecordRewardGrant update{};
    if (!queuez::stage_record_reward_grant(session.queuez, pending->accountSoid,
                                          pending->characterSoid,
                                          std::span(residents).first(residentCount), update))
        return refuse("F4 resident staging refused; nothing committed");
    touchesScratch = true;
    auto nextSendNonce = session.sendNonce;
    std::size_t framedSize = 0;
    if (!push::append_record_reward_notification(scratch, session.queuez, update, *pending,
                                                 active_acquisition_presentation_rows(session),
                                                 session.sessionKey, nextSendNonce,
                                                 scratch.framed, framedSize)
        || framedSize == 0)
    ...
    if (!state::commit_record_reward(*pending) || !transaction.commit()) {
        receipt.changed = receipt.inventoryCount = receipt.postmasterCount = receipt.materialCount = 0;
...
[[nodiscard]] bool consume_family5_refresh(Session& session,
    ...
    if (!push::append_family5_override_notification(
            scratch, session.queuez.publishedMoteMask, publishedMoteMask, version,
            session.sessionKey, nextSendNonce, scratch.framed, framedSize)
        || framedSize == 0 || framedSize > response.size()) {
        core::log::write(core::log::Channel::server,
                         core::log::Level::warn,
...
bool consume_deferred(Session& session,
    ...
    if (consume_synthesizer_family4_refresh(session, session.synthesizerFamily4Refresh,
                                          scratch, response, written, touchesScratch)) {
        return true;
    }
