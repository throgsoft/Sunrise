...
bool configure_banks(std::span<const std::byte> flagSlots,
    ...
    if (!array(flagSlots, 8, 0x80807D4FU, 8, flags) || !array(valueSlots, 8, 0x80807C96U, 8, values)
        || !array(flagMaps, 8, 0x80807D48U, 8, flagMap)
        || !array(valueMaps, 8, 0x80807C8DU, 8, valueMap))
        return false;
...
bool configure_sockets(std::span<const std::byte> bytes) noexcept {
    ...
    for (std::size_t lane = 0; lane < kSocketTypes.size(); ++lane) {
        ...
        if (kSocketTypes[lane] >= types.count || !read(bytes, at, hash)
...
bool configure_item(std::uint16_t index,
                    std::uint32_t hash,
    ...
    if (index == kChaliceIndex) {
        ...
        std::uint32_t cls{};
        if (!read(bytes, 64, relative) || relative <= 0 || bytes.size() < 64
            || static_cast<std::uint64_t>(relative) > bytes.size() - 64)
            return false;
        ...
        if (!tables::find_optional_array_at(bytes, at + 64, rules)
            ...
            || (rules.count != 0
                && (rules.elementClass != 0x808077E6U || rules.dataOffset > bytes.size()
                    || rules.count > (bytes.size() - rules.dataOffset) / 16)))
            return false;
        ...
        if (index >= 7949 && index <= 7984) {
            ...
            if (!expression(bytes, at + 224, quantity) || quantity.count != 1
                ...
                || quantity.code[0].operand != 5512U + (index - 7949U) % 12U)
                return false;
        }
    }
    output.hash = hash;
