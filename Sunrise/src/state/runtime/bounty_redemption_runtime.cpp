...
bool append(Requests& requests,
    ...
    if (detail.instancedDefinitionState == items::details::InstancedDefinitionState::stackable) {
        for (std::size_t i = 0; i < count; ++i) {
            if (requests[i].itemDefinitionIndex != index) {
                continue;
            }
...
bool rank_credit(RewardMarker marker, std::int32_t quantity, PendingRedemption& pending) noexcept {
    ...
    if (pending.rankCount == pending.ranks.size()) {
        return false;
    }
...
bool collect(AccountState& working,
    ...
    if (detail.rewardCount > detail.rewards.size()) {
        return false;
    }
    for (std::size_t entry = 0; entry < detail.rewardCount; ++entry) {
        const auto& reward = detail.rewards[entry];
        if (reward.itemIndex == items::details::kUnavailableItemIndex) {
            continue;
        }
        ...
        if (marker == RewardMarker::valorRankPoints || marker == RewardMarker::infamyRankPoints) {
            if (!rank_credit(marker, reward.quantity, pending)) {
                return false;
            }
            continue;
        }
        ...
        if (marker != RewardMarker::none) {
            const auto policy = resolve_reward_policy(source.definitionHash, marker);
            ...
            if (policy.lootPool == LootPoolId::gambitPrimeSynthesizer) {
                ...
                if (!upgradesSynthesizer && !synthesizer::stage_upgrade(character)) {
                    return false;
                }
            ...
            if (selected == 0 || !append_hash(requests, count, selected)) {
                return false;
            }
            continue;
        }
        ...
        if (!append(requests, count, paidIndex, quantity)) {
            return false;
        }
    }
    if (paysExperience) {
        pending.experience = cadence_experience(cadence) * (doubleExperience ? 2 : 1);
        if (pending.experience <= 0) {
            return false;
        }
...
bool stage(const AccountState& current,
    ...
    if (index == before.inventory.count) {
        return false;
    }
    ...
    if (!measured.resolved || measured.objectiveCount != detail.objectiveCount) {
        return false;
    }
    const auto now = core::runtime::investment_clock_seconds();
    if (grantTime <= 0 || grantTime > now) {
        return false;
    }
    ...
    if (transition) {
        if (!append_hash(requests, count, mapHash)) {
            return false;
        }
    ...
    if (!stage_reward_placement(working,
                                std::span(requests).first(count),
                                kUnclaimedRecordIndex,
                                {RewardPlacementPolicy::bountyRedemption, soid, grantTime},
                                pending.grant)) {
        return false;
    }
    if (transition) {
        // Only the instance this redemption granted takes the new deadline. Stamping every held
        // copy would silently extend maps the player earned separately.
        std::uint64_t grantedSoid = 0;
        for (std::size_t i = 0; i < pending.grant.rewardCount && i < pending.grant.rewards.size();
             ++i) {
            const auto& reward = pending.grant.rewards[i];
            if (reward.definitionHash != mapHash || reward.instanceSoid == source.instanceSoid) {
                continue;
            }
            if (grantedSoid != 0) {
                return false;
            }
            grantedSoid = reward.instanceSoid;
        }
        for (std::size_t i = 0;
             grantedSoid != 0 && i < pending.grant.afterCharacter.inventory.count;
             ++i) {
            auto& item = pending.grant.afterCharacter.inventory.values[i];
            if (item.instanceSoid == grantedSoid
...
bool materialize(const AccountState& current,
    ...
    if (!pending.prepared || !grant.prepared || current.primarySoid != grant.accountSoid
        || grant.claimedRecordIndex != kUnclaimedRecordIndex
        ...
        || !same_profile_inventory(
            current, grant.beforeProfileItems, grant.beforeProfileItemCount)) {
        return false;
    }
...
bool commit_redemption(PendingRedemption& pending) noexcept {
    ...
    for (std::size_t i = 0; i < pending.grant.rewardCount; ++i) {
        if (!seed_quest_initialization(pending.grant.rewards[i].definitionHash)) {
            return false;
        }
    }
    for (std::size_t i = 0; i < pending.rankCount; ++i) {
    ...
    if (pending.experience > 0 && !grant_seasonal_experience(pending.experience)) {
        return false;
    }
...
bool prepare_redemption_grant(std::uint64_t source,
    ...
    if (!pending || !prepare_redemption(source, quantity, *pending)) {
        return false;
    }
    grant = pending->grant;
...
bool materialize_redemption_grant(const AccountState& current,
                                  ...
                                  AccountState& after) noexcept {
    if (!grant.pursuitRedemption || !grant.pursuitRedemption->prepared) {
        return false;
    }
    auto pending = std::unique_ptr<PendingRedemption>{new (std::nothrow) PendingRedemption};
    if (!pending) {
...
bool prepare_bounty_redemption_grant(std::uint64_t sourceInstanceSoid,
                                     std::int32_t expectedQuantity,
                                     PendingRecordRewardGrant& grant) noexcept {
    return runtime::detail::bounty::prepare_redemption_grant(
        sourceInstanceSoid, expectedQuantity, grant);
}

bool is_bounty_reward_marker(std::uint16_t itemIndex, std::uint32_t definitionHash) noexcept {
    std::uint32_t installedHash{};
    return runtime::detail::bounty::reward_marker(itemIndex, installedHash)
               != runtime::detail::bounty::RewardMarker::none
           && installedHash == definitionHash;
}
