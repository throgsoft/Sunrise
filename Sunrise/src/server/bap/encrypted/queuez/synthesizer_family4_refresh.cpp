...
bool consume_synthesizer_family4_refresh(
    ...
    for (std::size_t i = 0; i < selected->inventory.count; ++i) {
        ...
        if (item.instanceSoid <= refresh.lastInstanceSoid || item.quantity != 1
            ...
            || !state::runtime::detail::synthesizer::is_container(item.definitionHash))
            continue;
    ...
    // Never introduce a new resident from a deferred view refresh. A resync owns that.
    ...
    if (residentMatches != 1) return false;
    ...
    if (!queuez::stage_equipment_swap(session.queuez, selected->soid, update)
        || !push::append_artifact_item_refresh_notification(
            scratch, update, target->instanceSoid, session.sessionKey, nextSendNonce,
    ...
    core::log::writef(core::log::Channel::server, core::log::Level::info,
                      "ev=synthesizer_visibility stage=item_refresh result=published "
                      "instance=0x%llX hash=%u family4_version=%d family5_version=%d",
                      static_cast<unsigned long long>(target->instanceSoid), target->definitionHash,
                      session.queuez.family4Version, session.queuez.family5Version);
    return true;
}
