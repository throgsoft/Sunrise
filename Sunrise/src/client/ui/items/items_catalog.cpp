...
Category category(const Entry& entry) noexcept {
    if (entry.itemType.find("Bounty") != std::string::npos) {
        return Category::bounties;
    }
    if (entry.itemType.find("Quest") != std::string::npos) {
    ...
    if (bucket.arraySelector == buckets::ArraySelector::profile) {
        ...
        if (bucket.bucketId == 28 || bucket.bucketId == 15) { // Consumables and modifications.
