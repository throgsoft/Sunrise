...
[[nodiscard]] bool read_definition(const reader::Source& source,
    ...
    ArrayView transfers{};
    if (read_array(blob, kTransferArrayDescriptor, 2, transfers)
        ...
        for (std::size_t row = 0; row < transfers.count; ++row) {
            ...
            (void)read(blob, transfers.base + row * 2, rule.sourceBucket);
            (void)read(blob, transfers.base + row * 2 + 1, rule.destinationBucket);
        }
    }
