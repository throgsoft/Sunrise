...
constexpr std::array<std::uint32_t, 4> kSocketTypes{
...
constexpr std::array<Role, 4> kRoles{{
    {3948022968U, {456408693U, 456408692U, 456408695U},
     {3611551832U, 3611551833U, 3611551834U}, {1431423016U, 1431423017U, 1431423018U}},
    {3552598030U, {3728290549U, 3728290548U, 3728290551U},
     {2657400314U, 2657400315U, 2657400312U}, {1401078730U, 1401078731U, 1401078728U}},
    {1691570586U, {3602650887U, 3602650886U, 3602650885U},
     {3162804766U, 3162804767U, 3162804764U}, {882903534U, 882903535U, 882903532U}},
    {889896758U, {1769115309U, 1769115308U, 1769115311U},
     {753747954U, 753747955U, 753747952U}, {248465794U, 248465795U, 248465792U}},
}};
...
std::array<SocketCost, kSocketTypes.size()> g_costs{};
bool g_costsReady{};
...
template <typename T>
bool read(std::span<const std::byte> bytes, std::size_t offset, T& value) noexcept {
...
bool single_cost(const items::Definition& plug,
    ...
    if (plug.insertionMaterialRequirementSetIndex == materials::kUnavailableSetIndex
        || !build_data::find_material_requirement_set(plug.insertionMaterialRequirementSetIndex, costs)
    ...
    return cost.itemDefinitionIndex == expectedItem && cost.quantity > 0
...
bool resolve_pair(const Role& role, std::size_t tier, ExchangePair& pair) noexcept {
    ...
    if (tier >= kContainers.size()
        || !profile_stack(role.synth, pair.synth, pair.synthDetail)
        ...
        || pair.recipe.definitionHash != role.plugs[tier] || pair.recipe.plugCategoryHash != 472268526U
        ...
        || !build_data::is_socket_plug_allowed(reference.definitionIndex,
                                               static_cast<std::uint8_t>(tier),
                                               pair.recipe.definitionIndex)
        || !build_data::is_socket_plug_allowed(reference.definitionIndex, 4, pair.recycle.definitionIndex))
        return false;
    ...
    if (!multiplier(tier, pair.synthesisSocketType, pair.synth.definitionIndex, scalar)
        ...
        || recycleScalar != 1
        || cost.quantity > static_cast<std::uint32_t>((std::numeric_limits<std::int32_t>::max)()) / scalar)
        return false;
...
bool exchange_profile(const AccountState& snapshot,
    ...
    for (std::size_t i = 0; i < candidate.profileItemCount; ++i) {
        ...
        if (item.definitionHash != reward.definitionHash) continue;
    ...
    if (!finalize_profile_item_acquisition(candidate, candidate, reward.definitionHash, reward,
                                            false, quantity, {.direct = true}, grant)
        || grant.acquiredInstanceSoid != 0 || grant.afterItems[grant.profileIndex].instanceSoid != 0)
        return false;
...
bool initialize_sockets(const items::details::Definition& detail,
    ...
    if (sockets.policy == inventory::SocketPolicy::nativeDefaults) {
        ...
        for (std::size_t lane = 0; lane < sockets.plugCount; ++lane) {
            if (detail.initialPlugIndices[lane] == items::details::kUnavailableItemIndex) continue;
...
bool configure_socket_costs(std::span<const std::byte> bytes) noexcept {
    const std::lock_guard lock{g_costMutex};
    g_costsReady = false;
    g_costs = {};
    tables::Array table{};
    if (!tables::find_array_at(bytes, 8, table)
        ...
        return false;
    std::array<SocketCost, kSocketTypes.size()> costs{};
    std::array<bool, kSocketTypes.size()> found{};
    for (std::size_t i = 0; i < table.count; ++i) {
        const auto at = table.dataOffset + i * tables::kSocketTypeRowStride;
        std::uint32_t hash{};
        if (!read(bytes, at, hash)) return false;
        const auto match = std::find(kSocketTypes.begin(), kSocketTypes.end(), hash);
        if (match == kSocketTypes.end()) continue;
        const auto tier = static_cast<std::size_t>(match - kSocketTypes.begin());
        if (found[tier]) return false;
        found[tier] = true;
        auto& cost = costs[tier];
        cost.socketType = static_cast<std::uint16_t>(i);
        tables::Array scalars{};
        if (!tables::find_optional_array_at(bytes, at + 56, scalars)
            || scalars.count > cost.scalars.size()
            || (scalars.count != 0
                && (scalars.elementClass != 0x80805D86U || scalars.dataOffset > bytes.size()
                    || scalars.count > (bytes.size() - scalars.dataOffset) / 8)))
            return false;
        ...
        for (std::size_t j = 0; j < cost.count; ++j) {
            ...
            if (!read(bytes, scalars.dataOffset + j * 8, scalar.itemIndex)
                || !read(bytes, scalars.dataOffset + j * 8 + 2, reserved) || reserved != 0
                || !read(bytes, scalars.dataOffset + j * 8 + 4, value)
            ...
            for (std::size_t prior = 0; prior < j; ++prior)
                if (cost.scalars[prior].itemIndex == scalar.itemIndex) return false;
        }
    }
    if (!std::all_of(found.begin(), found.end(), [](bool value) { return value; })) return false;
    g_costs = costs;
    g_costsReady = true;
    return true;
}
...
bool configure_mote_output_flags(std::uint32_t hash,
                                 std::span<const std::byte> bytes,
                                 std::span<const std::byte> flagSlots) noexcept {
    std::size_t chosen = g_moteOutputs.size();
    for (std::size_t role = 0; role < kRoles.size(); ++role)
        for (std::size_t tier = 0; tier < 3; ++tier)
            if (kRoles[role].motes[tier] == hash) chosen = role * 3 + tier;
    if (chosen == g_moteOutputs.size()) return false;
    const std::lock_guard lock{g_costMutex};
    g_moteOutputs[chosen] = {};
    ...
    if (!read(bytes, 144, relative) || relative <= 0
        || static_cast<std::uint64_t>(relative) > bytes.size()
        || bytes.size() - static_cast<std::size_t>(relative) < 144)
        return false;
    const auto block = 144 + static_cast<std::size_t>(relative);
    std::uint32_t cls{};
    tables::Array flags{}, slots{}, other{};
    if (!read(bytes, block - 4, cls) || cls != 0x808077ABU
        || !tables::find_array_at(bytes, block, flags)
        || flags.elementClass != 0x80807D4BU || flags.count != 2
        || flags.dataOffset > bytes.size() || bytes.size() - flags.dataOffset < 4
        || !tables::find_optional_array_at(bytes, block + 16, other) || other.count != 0
        || !tables::find_optional_array_at(bytes, block + 32, other) || other.count != 0
        || !tables::find_array_at(flagSlots, 8, slots) || slots.elementClass != 0x80807D4FU
        || slots.dataOffset > flagSlots.size()
        || slots.count > (flagSlots.size() - slots.dataOffset) / 8)
        return false;
    MoteOutputFlags output{};
    for (std::size_t i = 0; i < output.slots.size(); ++i) {
        std::uint8_t kind{}, scope{};
        std::uint16_t bank{};
        if (!read(bytes, flags.dataOffset + i * 2, output.slots[i])
            || output.slots[i] >= slots.count || output.slots[i] >= 23500)
            return false;
        const auto row = slots.dataOffset + static_cast<std::size_t>(output.slots[i]) * 8;
        if (!read(flagSlots, row + 4, kind) || kind != 0
            || !read(flagSlots, row + 5, scope) || scope != 0
            || !read(flagSlots, row + 6, bank) || bank != 0xFFFFU)
            return false;
    }
    if (output.slots[0] == output.slots[1]) return false;
    output.configured = true;
    g_moteOutputs[chosen] = output;
    return true;
}
...
bool project_mote_output_flags(const AccountState& account, Family5State& family,
                               std::uint16_t previousMoteMask) noexcept {
    ...
    if (!output_flags_ready(outputs)) return false;
    ...
    for (std::size_t i = 0; i < outputs.size(); ++i) {
        ...
        for (const auto slot : outputs[i].slots) {
            std::size_t at = 0;
            while (at < count && derived[at].slot != slot) ++at;
            if (at == count) {
                if (count == derived.size()) return false;
                // Native 549450 merges the first nonzero source at each slot: zero
            ...
            if (held) derived[at].value = 2;
            const bool alreadyProjected = std::any_of(
                family.flags.begin(), family.flags.begin() + family.flagCount,
                [slot](const auto& flag) { return flag.slot == slot; });
            publish[at] = publish[at] || held || alreadyProjected
                          || (previousMoteMask & (1U << i)) != 0;
        }
    }
    if (count != derived.size()) return false;
    auto after = family;
    ...
    for (std::size_t i = 0; i < family.flagCount; ++i) {
        ...
        if (!replaced) after.flags[after.flagCount++] = row;
    }
    const auto publishedCount = static_cast<std::size_t>(std::count(publish.begin(), publish.end(), true));
    if (publishedCount > after.flags.size() - after.flagCount) return false;
    ...
    for (std::size_t i = 0; i < derived.size(); ++i)
        if (publish[i]) after.flags[after.flagCount++] = derived[i];
    family = after;
    return true;
}
...
bool project_exchange(const PendingSocketPlug& mutation,
    ...
    if (!mutation.prepared || !mutation.profileChanged || !is_container(mutation.targetDefinitionHash)
        || mutation.accountSoid != object.accountSoid
        || mutation.accountSoid != after.primarySoid
        ...
        || !same_profile_inventory(after, mutation.afterProfileItems, mutation.afterProfileItemCount))
        return false;
    ...
    if (!recycling && mutation.socketLane >= 3) return false;
    ExchangePair pair{};
    bool found = false;
    for (const auto& role : kRoles) {
        for (std::size_t tier = 0; tier < kContainers.size(); ++tier) {
            ...
            if (!build_data::find_item_definition_hash(
                ...
                continue;
            if (found || (!recycling && tier != mutation.socketLane) || !resolve_pair(role, tier, pair))
                return false;
            found = true;
        }
    }
    ...
    for (std::size_t i = 0; i < mutation.expectedProfileItemCount; ++i) {
        ...
        if (item.definitionHash == debit.definitionHash) debitBefore += item.quantity;
        if (item.definitionHash == credit.definitionHash) creditBefore += item.quantity;
    }
    for (std::size_t i = 0; i < mutation.afterProfileItemCount; ++i) {
        const auto& item = mutation.afterProfileItems[i];
        if (item.definitionHash == debit.definitionHash) debitAfter += item.quantity;
        if (item.definitionHash != credit.definitionHash) continue;
    ...
    for (std::size_t slot = 0; slot < object.profileItems.size(); ++slot) {
        ...
        // balances must occupy the authored bucket window used by native 549E50.
        if ((row.definitionIndex == debit.definitionIndex || row.definitionIndex == credit.definitionIndex)
            && (slot < bucket.firstSlot || slot - bucket.firstSlot >= bucket.slotCount))
            return false;
        if (row.definitionIndex == debit.definitionIndex) {
            if (row.instanceSoid != 0 || row.quantity <= 0) return false;
            encodedDebit += row.quantity;
        }
        if (row.definitionIndex != credit.definitionIndex) continue;
    ...
    core::log::writef(core::log::Channel::state,
                      ...
                      unsigned(recycling), unsigned(credit.definitionIndex), gain->quantity,
                      gain->mutationSerial, gainSlot, unsigned(bucket.bucketId),
                      unsigned(mote_ownership_mask(std::span(after.profileItems).first(after.profileItemCount))),
                      unsigned(previousMoteMask), projectedFlagCount);
    return true;
}
...
bool stage_exchange(const AccountState& snapshot,
    ...
    if ((socketLane >= 3 && !recycling) || !account::valid(snapshot) || !valid_profile_inventory(snapshot)
        || characterIndex >= snapshot.characterCount || !snapshot.characters[characterIndex].selected)
        return false;
    ...
    if (!target || !is_container(target->definitionHash) || target->quantity != 1
        ...
        || detail.definitionIndex != container.definitionIndex || detail.bucketId != container.bucketId
        ...
        || !build_data::is_socket_plug_allowed(container.definitionIndex, socketLane, plugDefinitionIndex)
    ...
    for (const auto& row : kRoles) {
        for (std::size_t i = 0; i < kContainers.size(); ++i) {
            if ((recycling ? row.recyclePlugs[i] : row.plugs[i]) != requested.definitionHash) continue;
            if (role || (!recycling && i != socketLane)) return false;
            role = &row;
            tier = i;
        }
    }
    ...
    if (!resolve_pair(*role, tier, pair)
        ...
        || (recycling && (!multiplier(3, detail.socketTypes[4], pair.mote.definitionIndex, recycleScalar)
                          || recycleScalar != 1)))
        return false;
    ...
    const auto quantity = recycling
                              ? static_cast<std::int32_t>(pair.synthesisCost.requirements[0].quantity)
                              : 1;
    if (!recycling) {
        buckets::Descriptor bucket{};
        if (!build_data::find_inventory_bucket_descriptor(pair.mote.bucketId, bucket)) return false;
        ...
        for (std::size_t i = 0; i < snapshot.profileItemCount; ++i) {
            ...
            for (const auto& other : kRoles)
                if (item.definitionHash == other.motes[tier]) return false;
            items::Definition definition{};
            if (!build_data::find_item_definition_hash(item.definitionHash, definition)) return false;
            if (definition.bucketId == pair.mote.bucketId) ++occupied;
        }
    ...
    if (!initialize_sockets(detail, sockets)) return false;
    sockets.plugs[socketLane] = initial.definitionHash;
    AccountState candidate = snapshot;
    if (!exchange_profile(snapshot, costs, reward, quantity, candidate)) return false;
    ...
    if (mote_output_flags_ready()) {
        ...
        if (!investment_snapshot(preview)
            || !project_mote_output_flags(candidate, preview.family5,
                mote_ownership_mask(std::span(snapshot.profileItems).first(snapshot.profileItemCount))))
            return false;
    }
