...
build(package::PresentationReader& reader, bool packagesReady, std::stop_token stop) {
    ...
    for (std::size_t first = 0; first < data::objective_definition_count(); first += 32) {
        ...
        const auto end =
            (std::min)({first + 32, data::objective_definition_count(), objectives.size()});
    ...
    for (std::size_t first = 0; first < count; first += 8) {
        ...
        for (auto i = first; i < end; ++i) {
            ...
            data::items::details::Definition detail{};
            entry.detailReady =
                data::find_configured_item_detail(entry.identity.definitionIndex, detail)
                && detail.definitionHash == entry.identity.definitionHash
                && detail.bucketId == entry.identity.bucketId;
            if (entry.detailReady) {
                entry.bounty = detail.bucketId == data::items::kPursuitBucketId
                               && detail.lifetimeSeconds > 0 && detail.objectiveCount > 0
                               && detail.maxStackSize <= 1;
                entry.quantityLimit =
                    detail.objectiveCount ? 1
        ...
        if (needCatalog) {
            ...
            g_status = g_catalog
                           ? (readable ? "Catalog ready."
                                       : "Package text and icons unavailable; Reload retries.")
