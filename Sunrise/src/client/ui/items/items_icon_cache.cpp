...
void begin_frame(ID3D11Device* device) noexcept {
    ...
    for (int count = 0; count < 2; ++count) {
        ...
        for (auto& slot : g_slots) {
            ...
            if (result.available) {
                (void)client::hooks::graphics::textures::upload_rgba8(g_device,
                                                                      result.icon.width,
                                                                      result.icon.height,
                                                                      result.icon.width * 4U,
                                                                      result.icon.rgba,
                                                                      slot.gpu);
            }
...
ImTextureID get(std::uint16_t index) noexcept {
    ...
    for (auto& slot : g_slots) {
        if (slot.index == index) {
            slot.used = g_frame;
            return slot.gpu.view
                       ? static_cast<ImTextureID>(reinterpret_cast<std::uintptr_t>(slot.gpu.view))
                       : ImTextureID_Invalid;
        }
