...
bool array(std::span<const std::byte> bytes, std::uint32_t cls, std::size_t stride,
...
PresentationReader::~PresentationReader() noexcept { reader::close_files(scratch_); }

bool PresentationReader::read(std::uint32_t tag, std::size_t limit,
    ...
    if (!reader::resolve_latest(scratch_, source_.directory, package, location) || !location
        || !reader::block_cache::load_header(location->latestPath, package,
                                             location->patchIndex, scratch_, header)
        ...
        || reader::layout::placement(entry).size > limit) return false;
...
bool PresentationReader::initialize() noexcept {
    ...
    const bool ready = read(tables::kItemStringsIndexTag, kTableLimit, strings_, cls)
                       ...
                       && cls == text::kStringBankIndexClass
                       && array(banks_, 0x80805F9EU, 8, rows);
    ...
    if (!read(kIconsTag, kTableLimit, icons_, cls) || cls != 0x80802951U
        || !array(icons_, 0x80802957U, 24, rows)) icons_.clear();
    if (!read(kObjectivesTag, kTableLimit, objectives_, cls) || cls != 0x808059ECU
        || !array(objectives_, 0x808059F0U, 64, rows)) objectives_.clear();
    return ready;
}
...
text::Reference PresentationReader::reference(std::span<const std::byte> bytes,
    ...
    if (!tables::read(bytes, offset, bank) || bank == 0xFFFF
        ...
        || !tables::read(std::span<const std::byte>{banks_},
                         rows.dataOffset + bank * 8 + 4, output.containerTag)) return {};
    return output;
}
...
bool PresentationReader::item(std::uint16_t index, std::uint32_t hash, Display& output) noexcept {
    ...
    if (!array(strings_, tables::kItemStringsIndexRowClass, 24, rows)
        ...
        || !read(row.targetTag, 64 * 1024, blob_, cls) || cls != 0x80805CE1U) return false;
...
bool PresentationReader::objective(std::uint16_t index, std::uint32_t hash,
...
bool PresentationReader::read_text(void* context, std::uint32_t tag,
...
bool PresentationReader::icon(std::uint16_t index, Icon& output) noexcept {
    ...
    if (index == kNoIcon || !array(icons_, 0x80802957U, 24, rows)
        ...
        || !read(row.targetTag, 128, blob_, cls) || cls != 0x80804A53U
        || blob_.size() != 128
    ...
    if (!tables::read(set, 8, kind) || kind != 0
        || !tables::read(set, 28, resourceClass) || resourceClass != 0x80804A67U
        || !tables::find_array_at(set, 32, outer) || outer.count != 1
        || (outerClass = outer.elementClass) != 0x80804A6CU
        ...
        || !read(tag, 40, blob_, cls) || blob_.size() != 40) return false;
    ...
    if (!tables::read(header, 0, size) || !tables::read(header, 4, format) || format != 28
        ...
        || !tables::read(header, 18, depth) || depth != 1
        || !tables::read(header, 20, layers) || layers != 1
        || !tables::read(header, 36, large) || large != 0xFFFFFFFFU
