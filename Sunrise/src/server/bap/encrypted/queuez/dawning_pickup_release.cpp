...
namespace {
bool append_pickups(Session& session, Scratch& scratch,
                    std::array<std::byte, state::kBapNonceSize>& nonce,
                    std::size_t& size, queuez::SessionState& after) noexcept {
    ...
    if (!selectedIndex || !snap::resolve(*account, *selectedIndex, resolved)
        ...
        || !queuez::stage_equipment_swap(session.queuez, account->characters[*selectedIndex].soid, update))
        return false;
    const auto bytes = std::span(scratch.plaintext).first(character::layout::kObjectSize);
    if (!character::encode(account->characters[*selectedIndex], resolved.loadout,
                           resolved.lightEvaluation, bytes)) return false;
    ...
    for (const auto& row : object.inventoryItems) {
        ...
        if (ingredient == identity::kIngredientCount || item.definitionHash != identity::kIngredients[ingredient].pickupHash)
            continue;
        if (row.instanceSoid != 0 || row.quantity != 1 || count == object.inventoryChanges.records.size())
            return false;
    ...
    object.inventoryChanges.writeSlot = object.inventoryChanges.nextSequence = static_cast<std::uint16_t>(count);
    snap::Prepared prepared{};
    std::size_t extent{};
    if (!snap::append_object(scratch, bytes, resolved.characterObjectId,
            account->characters[*selectedIndex].soid, prepared.objects[0], extent)) return false;
    prepared.rawClearSize = bytes.size();
    prepared.compressedClearSize = extent;
    prepared.family = {queuez::kAccountFamilyType, account->primarySoid, update.after.family4Version,
                       0, std::span(prepared.objects).first(1)};
    if (!push::queuez_frame::append_prepared_frame(scratch, prepared, session.sessionKey, nonce,
                                                  scratch.framed, size)) return false;
    after = update.after;
    return true;
}
}

bool consume_dawning_pickup_release(Session& session, Scratch& scratch,
    ...
    std::size_t acquired{};
    if (removed == 0 && !state::runtime::detail::dawning::stage_queued_pickups(selected->soid, acquired))
        return false;
    ...
    const bool encoded = acquired != 0 ? append_pickups(session, scratch, nonce, size, after)
        : push::append_account_resync_notification(scratch, session.queuez, {}, session.sessionKey,
            nonce, scratch.framed, size, after);
    if (!encoded
        || size == 0 || size > response.size() || !queuez::valid(after)
        || !transaction.commit())
        return false;
    ...
    core::log::writef(core::log::Channel::server, core::log::Level::info,
        ...
        acquired ? "published" : "released", after.family4Version, acquired ? acquired : removed,
