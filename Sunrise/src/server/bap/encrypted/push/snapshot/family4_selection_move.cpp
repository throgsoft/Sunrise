...
bool prepare_postmaster_claim(Scratch& scratch,
    ...
    if (!family4_datagen::character::encode(account->characters[mutation.characterIndex],
                                            selected.loadout, selected.lightEvaluation, bytes))
    ...
    return finish_character_upsert(scratch, reservation, bytes, update,
                                    "postmaster_object", "postmaster_snapshot", prepared);
}
