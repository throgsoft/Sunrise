...
[[nodiscard]] bool
relative(std::span<const std::byte> blob, std::size_t field, std::size_t& output) noexcept {
    std::int64_t delta = 0;
    ...
    for (std::uint64_t groupIndex = 0; groupIndex < groups.count; ++groupIndex) {
        ...
        for (std::size_t tableIndex = 0; tableIndex < kActivityTableCount; ++tableIndex) {
            ...
            const std::size_t descriptor =
                group + kActivityTableFirstDescriptor + tableIndex * 0x10;
...
[[nodiscard]] bool find_string_index(std::span<const std::byte> localized,
    ...
    if (low >= hashes.count
        || !read(
            localized, hashes.dataOffset + static_cast<std::size_t>(low) * kStringHashStride, value)
        || value != wanted) {
        return false;
    }
...
[[nodiscard]] bool decode_string(std::span<const std::byte> data,
    ...
    if (parts.elementClass != kStringPartClass
        || combinations.elementClass != kStringCombinationClass || stringIndex >= combinations.count
    ...
    const std::size_t combination =
        combinations.dataOffset + stringIndex * kStringCombinationStride;
...
bool append(std::wstring_view packageDirectory,
    ...
    const bool resolved =
        collected && resolve_candidates(source, *scratch, candidates, output, result);
