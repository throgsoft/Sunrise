...
/** Settling interval shared by the initial arm and every deferred retry. */
inline constexpr std::uint64_t kSettlingMilliseconds = 100;
...
inline void arm_synthesizer_family4_refresh(SynthesizerFamily4Refresh& refresh,
    ...
    refresh = {now + kSettlingMilliseconds, 0, true};
}

[[nodiscard]] bool consume_synthesizer_family4_refresh(Session& session,
